...
// นำเข้า Component ย่อยที่แยกไฟล์ไว้ เพื่อนำมาแสดงผลแบบสลับหน้าจอ (Conditional Rendering)
import Navbar from "./components/05_Navbar";
import Surface from "./components/01_Surface";
import Supplies from "./components/06_Supplies";
import Radar from "./components/07_Radar";
import Logbook from "./components/08_Logbook";

// นำเข้าไอคอนจากไลบรารี lucide-react เพื่อใช้ประกอบ UI ให้สื่อความหมายมากขึ้น
import {
  Siren,
  MessageCircleHeart,
  Send,
...
export default function App() {
  ...
  // State สำหรับคุมการเปลี่ยนหน้าเว็บ (ทำงานคล้าย Router)
  // กำหนดค่าเริ่มต้นเป็น "mission" เพื่อให้โหลดแอปแล้วเจอหน้าภารกิจกู้ภัยทันที
  const [activePage, setActivePage] = useState("mission");

  // State สำหรับจัดการข้อมูลภายในหน้าภารกิจกู้ภัย (Mission)
  ...
  // ==========================================
  // 2. ฟังก์ชันควบคุมการทำงาน (Logic Functions)
  // ==========================================

  // ฟังก์ชันล้างค่า State ทั้งหมด เพื่อให้ผู้เล่นสามารถเริ่มภารกิจใหม่ได้
  const handleReset = () => {
  ...
  // ฟังก์ชันควบคุมลำดับเหตุการณ์ของ Story Modal
  // มีการเรียกใช้ Fetch API เพื่อสุ่มดึงข้อมูลโปเกมอนเมื่อเข้าสู่สถานะ calling
    ...
    if (rescueStatus === "calling") {
      ...
      fetch(`https://pokeapi.co/api/v2/pokemon/${randomId}`)
        ...
        .then((data) => {
          ...
          setRescueStatus("recruiting"); // เปลี่ยนสถานะเป็นได้ฮีโร่แล้ว
        });
    } else if (rescueStatus === "recruiting") {
      setRescueStatus("traveling"); // เปลี่ยนสถานะเป็นกำลังเดินทาง
    } else if (rescueStatus === "traveling") {
      setRescueStatus("success"); // เปลี่ยนสถานะเป็นช่วยเหลือสำเร็จ
    }
  };

  // ฟังก์ชันจัดการเมื่อ Lola ส่งข้อความ
  // ทำการตรวจสอบเงื่อนไข หากมีคำว่า "help" หรือ "ช่วยด้วย" จะกระตุ้นให้สถานะกู้ภัยทำงาน
  ...
  // ฟังก์ชันรับข้อความคำสั่งจาก Gromit ไปแสดงผล
  ...
  // ตัวแปรแบบ Boolean เพื่อตรวจสอบว่าสถานะปัจจุบันจำเป็นต้องแสดง Popup เนื้อเรื่องหรือไม่
  const isStoryActive = ["calling", "recruiting", "traveling"].includes(
    rescueStatus,
  );

  // ==========================================
  // 3. ส่วนแสดงผลหน้าจอ (UI Rendering)
  // ==========================================
  return (
    // โครงสร้างหลักของแอป กำหนดสีพื้นหลังและฟอนต์ให้เป็นมาตรฐานเดียวกัน
    <div className="min-h-screen bg-[#f0f2f5] flex flex-col items-center py-6 sm:py-10 px-4 font-sans relative text-slate-800">
      {/* Component Navbar จะแสดงผลเสมอเพื่อใช้เป็นเมนูหลัก โดยรับ Props เป็น state เพื่อจัดการสีปุ่ม */}
      <Navbar activePage={activePage} setActivePage={setActivePage} />

      {/* ใช้เงื่อนไข Logical AND (&&) ในการทำ Conditional Rendering เพื่อแสดง Component ตามค่าของ activePage */}

      {/* หน้า Home: แนะนำแอปพลิเคชัน */}
      {activePage === "home" && (
        <div className="w-full max-w-xl bg-white shadow-sm rounded-[2.5rem] p-10 text-center animate-in fade-in duration-500 border border-slate-100">
          ...
          <h1 className="text-3xl font-black text-slate-700 mb-4 tracking-wider uppercase">
            Gromit HQ
          </h1>
          <p className="text-slate-500">
            Welcome to the command center. Select a module to start.
          </p>
        </div>
      )}

      {/* หน้า Mission: รวมโค้ดแอปพลิเคชันกู้ภัยหลัก */}
      {activePage === "mission" && (
        <div className="w-full flex flex-col items-center animate-in fade-in duration-500">
          {/* ส่วน Commander Station (แผงควบคุมของ Gromit) */}
          ...
          {/* ส่วน Story Modal (แสดง Popup ซ้อนทับเนื้อหาหลักเมื่อตัวแปร isStoryActive เป็น true) */}
          {isStoryActive && (
          ...
          {/* ส่วนประกอบด้านล่าง ส่ง Props ต่อไปยัง Component ลูก */}
          <Surface
      ...
      {/* หน้าแบบฝึกหัด Component ย่อยต่างๆ (ดึงจากไฟล์แยกเพื่อความเป็นระเบียบ) */}
      {activePage === "supplies" && <Supplies />}

      {activePage === "radar" && <Radar />}

      {activePage === "logbook" && <Logbook />}
    </div>
  );
}
